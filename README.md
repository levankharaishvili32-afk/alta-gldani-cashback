...
renders nothing. To add slides, drop a desktop and a mobile crop into
`public/banners/` and append an entry (see the `Banner` type in
`lib/banners.ts`) — no component changes needed.

Next.js (App Router) + Tailwind CSS v4. The catalog is a local JSON file imported from the shop's
...
1. **Push to GitHub.** From the project folder:
   ...
   ```bash
   git add -A
   git commit -m "ALTA Gldani campaign landing"
   git branch -M main
